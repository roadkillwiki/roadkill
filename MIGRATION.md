...
- Nouvelle page « Pages by tag » (`/pages/alltagswithpages`) : chaque tag avec ses pages, repliable (`<details>`), tri par
  nombre de pages (défaut) ou par nom (`?sort=name`), boutons tout déplier / tout replier, pages sans tag en bas, recherche
  dans les titres côté client (déplie les tags trouvés), nombre de tags de chaque page (liste en info-bulle) ; code de menu `%tagswithpages%`
