...
Attacklab.wmdBase = function () {
	...
	wmd.PanelCollection = function () {
		...
		// FIX FOR Roadkill
		if ($(".wmd-textarea").length < 1)
			alert("Unable to find a textarea with a .wmd-textarea class");
		else
			this.input = $(".wmd-textarea")[0];
	};
	...
	util.isVisible = function (elem) {
		...
		if (window.getComputedStyle) {
			// Most browsers
			//if (window.getComputedStyle(elem, null) != null && window.getComputedStyle(elem, null).getPropertyValue("display") != null)
	...
	util.prompt = function (text, defaultInputText, makeLinkMarkdown) {
		...
		var close = function (isCancel) {
			...
			if (isCancel) {
				...
				// FIX FOR Roadkill (check for url starting with /)
				if (text.indexOf("/") != 0) {
		...
		var createDialog = function () {
			...
			dialog.style.border = "1px solid #999999"; // FIX FOR Roadkill
			dialog.style.backgroundColor = "#F5F5F5"; // FIX FOR Roadkill
			...
			input.id = "wmddialoginput"; // ADDED FOR Roadkill
			style = input.style;
			style.display = "block";
			style.width = "80%";
			...
			// ---- ADDED FOR Roadkill
			var roadkillChooser = doc.createElement("input");
			roadkillChooser.type = "button";
			roadkillChooser.value = "...";
			window.onFileSelected = function (url) {
				input.value = url;
			}
			window.srcField = "xyz";
			//roadkillManagerUrl = "Page/AllFiles/"; // this is set in the edit view
			roadkillChooser.onclick = function () {
				window.open(roadkillManagerUrl, "roadkillChooser", "width=400,height=500,status=0,toolbar=0");
			}

			style = roadkillChooser.style;
			style.margin = "10px";
			style.display = "inline";
			style.width = "35px";

			form.appendChild(roadkillChooser);
			// ---- END ADDED FOR Roadkill
