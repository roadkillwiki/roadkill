...
//   dotnet run ConvertWebConfig.cs -- <path to web.config> [path to appsettings.json]
//
// Without the second argument, appsettings.json is written in the folder of this script. If the appsettings.json file exists,
// only its "ConnectionStrings:Roadkill" value and "Roadkill" section are replaced, its other settings are kept.
...
return Migration.Run(args[0], args.Length > 1 ? args[1] : null, GetScriptDirectory());

// The folder of this script (the path of the source file, set by the compiler)
static string GetScriptDirectory([System.Runtime.CompilerServices.CallerFilePath] string path = "") => Path.GetDirectoryName(path)!;
