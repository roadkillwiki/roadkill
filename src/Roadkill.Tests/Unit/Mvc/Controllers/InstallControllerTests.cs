...
namespace Roadkill.Tests.Unit
{
	...
	public class InstallControllerTests
	{
		...
		public void TestAttachments_Should_Return_JsonResult_And_TestResult_Model_Without_Errors()
		{
			...
			Assert.That(testResult.ErrorMessage, Is.Empty);
		}
		...
		public void TestAttachments_Should_Return_TestResult_Model_With_Errors_When_Directory_Is_Not_Writeable()
		{
			...
			// Act
			ActionResult result = _installController.TestAttachments(@"J:\Windows\Microsoft.NET\Framework64\v4.0.30319");
			...
			Assert.That(testResult.ErrorMessage, Is.Not.Null.Or.Empty);
		}
		...
		{
			// Arrange
			SetMockDataStoreType(new SettingsViewModel());

			// Act
			ActionResult result = _installController.TestDatabaseConnection("connectionstring", "mock datastore");
		...
		{
			// Arrange
			string errorMessage = RepositoryThrowsExceptionsMock.ExceptionMessage;
			SetMockDataStoreType(new SettingsViewModel());
			...
			TestResult testResult = jsonResult.Data as TestResult;
			Assert.That(testResult, Is.Not.Null);
			Assert.That(testResult.ErrorMessage, Is.StringContaining(errorMessage));
		}
