...
namespace Roadkill.Core.Controllers
{
	...
	public class FilesController : ControllerBase
	{
		...
		public ActionResult DeleteFile(string filePath)
		{
			try
			{
				string folder = ApplicationSettings.AttachmentsDirectoryPath;
		...
		{
			FileSummary summary = FileSummary.FromBase64UrlPath(id, ApplicationSettings);
		...
		{	
			...
			SiteSettings siteSettings = _settingManager.GetSiteSettings();

			if (siteSettings.AllowedFileTypesList.FirstOrDefault(e => e.ToLower() == extension.ToLower()) != null)
			{
				try
				{
					DirectorySummary summary = DirectorySummary.FromBase64UrlPath(ApplicationSettings, currentUploadFolderPath);

					if (!Directory.Exists(ApplicationSettings.AttachmentsDirectoryPath))
						Directory.CreateDirectory(ApplicationSettings.AttachmentsDirectoryPath);
